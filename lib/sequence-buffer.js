...
function reliable_sequence_buffer_create(num_entries, allocate_function) {
...
function reliable_sequence_buffer_remove_entries(sequence_buffer,
  ...
  if (finish_sequence < start_sequence)
    finish_sequence = uint16_increment(finish_sequence, 65535)

  if (uint16_increment(finish_sequence, -start_sequence) < sequence_buffer.num_entries) {
    for (scratch[0] = start_sequence; scratch[0] <= finish_sequence; ++scratch[0]) {

      // TODO; this doesnt belong here, there should be a deallocator function
      if(sequence_buffer.entry_data[scratch[0] % sequence_buffer.num_entries].packet_data)
        pool.free(sequence_buffer.entry_data[scratch[0] % sequence_buffer.num_entries].packet_data)

      sequence_buffer.entry_sequence[ scratch[0] % sequence_buffer.num_entries ] = EMPTY
    }
  }
  else {
    for (let i = 0; i < sequence_buffer.num_entries; ++i) {
      ...
      // TODO; this doesnt belong here, there should be a deallocator function
      if(sequence_buffer.entry_data[i].packet_data)
        sequence_buffer.entry_data[i].packet_data
...
function reliable_sequence_buffer_remove(sequence_buffer, sequence) {
  ...
  // TODO; this doesnt belong here, there should be a deallocator function
  if(sequence_buffer.entry_data[sequence % sequence_buffer.num_entries].packet_data)
    pool.free(sequence_buffer.entry_data[sequence % sequence_buffer.num_entries].packet_data)
