...
function reliable_sequence_buffer_create(num_entries, entry_data_allocator) {
  ...
  for (let i=0; i < num_entries; i++) {
    if(entry_data_allocator) {
      entry_data.push(JSON.parse(JSON.stringify(entry_data_allocator)))
    } else {
      entry_data.push({
        acked: false,
        sent_time: 0,
        message: pool.malloc(c.MAX_PACKET_SIZE),
        byteLength: 0,
        includedMessages: []
      })
    }
...
function reliable_sequence_buffer_remove_entries(sequence_buffer,
  ...
  if (uint16_increment(finish_sequence, -start_sequence) < sequence_buffer.num_entries) {
    const sequence = new Uint16Array(1)
    for (sequence[0] = start_sequence; sequence <= finish_sequence; ++sequence[0])
      sequence_buffer.entry_sequence[ sequence[0] % sequence_buffer.num_entries ] = EMPTY
  }
  else {
