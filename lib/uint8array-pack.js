...
function pack_uint16(arr, index, num) {
  arr[index] = num >> 8
  arr[index+1] = num & 0xff
}

function pack_uint32 (arr, index, num) {
  const n = num & 0xffffffff
  arr[index] = ((n & 0xff000000) >>> 24)
  arr[index+1] = ((n & 0x00ff0000) >>> 16)
  arr[index+2] = ((n & 0x0000ff00) >>>  8)
  arr[index+3] = ((n & 0x000000ff))
}
...
module.exports = function pack(dest) {
  ...
  const packArray = function(src, count=src.byteLength) {
    // https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/TypedArray/set
    //dest.set(src, index)

    copyBytes(dest.buffer, src.buffer, index, 0, count)
    index += count
  }

  const packInt16 = function(val) {
    if(index + 2 > dest.byteLength) {
      console.error('packInt16: out of range')
      return
    }
  ...
  const packFloat32 = function(val) {
    if(index + 4 > dest.byteLength) {
      console.error('packFloat32: out of range')
      return
    }
  ...
  const packFloat64 = function(val) {
    if(index + 8 > dest.byteLength) {
      console.error('packFloat64: out of range')
      return
    }
  ...
  const packString = function(val) {
    if(index + 1 + val.byteLength > dest.byteLength) {
      console.error('packString: out of range')
      return
    }
  ...
  const packUint8 = function(val, idx) {
    ...
    if(index + 1 > dest.byteLength) {
      console.error('packUint8: out of range')
      return
    }
  ...
  const packUint16 = function(val, idx) {
    ...
    if(index + 2 > dest.byteLength) {
      console.error('packUint16: out of range')
      return
    }
  ...
  const packUint32 = function(val, idx) {
    ...
    if(index + 4 > dest.byteLength) {
      console.error('packUint32: out of range')
      return
    }
  ...
  return { packArray, packFloat32, packFloat64, packInt16, packString, packUint8, packUint16, packUint32, reset }
}
